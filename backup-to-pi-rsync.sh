...
# Bash (CentOS) Version (changed mount process - no fstab)
...
SOURCE1=/home/james/Documents
SOURCE2=/home/james/Pictures
DESTINATION=$REMOTEMOUNT/backup/$THISHOST
LOGFILE=$SOURCE1/`basename $0`.log
PID=/tmp/`basename $0`.pid
EXCLUDE=$SOURCE1/documents-rsync-exclude.txt
CONNECTSTRING=""
...
rsync --archive --no-perms --no-group --human-readable --exclude-from $EXCLUDE --log-file=$LOGFILE --rsh="ssh -p 22231 -l james" $SOURCE1 $SOURCE2 $DESTINATION
